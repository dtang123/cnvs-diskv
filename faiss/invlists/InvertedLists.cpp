...
void ClusteredArrayInvertedLists::permute_invlists(const idx_t* map){
    std::vector<MaybeOwnedVector<uint8_t>> new_codes(nlist);
    std::vector<MaybeOwnedVector<idx_t>> new_ids(nlist);
    std::vector<std::vector<size_t>> new_maps(nlist);

    for (size_t i = 0; i < nlist; i++) {
        size_t o = map[i];
        FAISS_THROW_IF_NOT(o < nlist);

        new_codes[i] = std::move(codes[o]);
        new_ids[i] = std::move(ids[o]);
        new_maps[i] = std::move(inlist_maps[o]);
    }
    codes = std::move(new_codes);
    ids = std::move(new_ids);
    inlist_maps = std::move(new_maps);
}
