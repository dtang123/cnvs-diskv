...
static IndexIVFPQ* read_ivfpq(IOReader* f, uint32_t h, int io_flags) {
    ...
    IndexDiskV* ivfpqd2 =
            (h == fourcc("IQD2") || h == fourcc("IDKV")) ? new IndexDiskV() : nullptr;

    //IndexIVFPQ* ivpq = ivfpqr ? ivfpqr : (ivfpqd ? ivfpqd : (ivfpqd2 ? ivfpqd2 : new IndexIVFPQ()));
    IndexIVFPQ* ivpq = ivfpqr ? ivfpqr : (ivfpqd2 ? ivfpqd2 : new IndexIVFPQ());
    ...
    if (ivpq->is_trained) {
        ...
        if (ivfpqr) {
            ...
            ivfpqd2->aligned_cluster_info = new Aligned_Cluster_Info[nlist]; // Allocate array dynamically based on nlist
            for (size_t i = 0; i < nlist; ++i) {
...
Index* read_index(IOReader* f, int io_flags) {
    ...
    } else if (h == fourcc("IwPf") || h == fourcc("IPfd")) {
        IndexIVFPQFastScan* ivpq = nullptr;
    
        // if (h == fourcc("IPfd")) {
        //     ivpq = new IndexIVFPQFastScanDisk();
        // } else {
        //     ivpq = new IndexIVFPQFastScan();
        // }
        ivpq = new IndexIVFPQFastScan();
