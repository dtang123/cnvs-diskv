...
void write_index(const Index* idx, IOWriter* f, int io_flags) {
    if (idx == nullptr) {
        ...
        const IndexDiskV* ivfpqd2 = dynamic_cast<const IndexDiskV*>(idx);
        uint32_t h;
        if (ivfpqr) {
            h = fourcc("IwQR");
        } 
        ...
        if (ivfpqr) {
            ...
            if(ivfpqd2->select_lists){
                ...
                std::cout << ivfpqd2->select_lists_path << "\n";
        ...
        //const IndexIVFPQFastScanDisk* ivfpqfsd = dynamic_cast<const IndexIVFPQFastScanDisk*>(idx);
        uint32_t h;
        // if (ivfpqfsd) {
        //     h = fourcc("IPfd");
        // } else {
