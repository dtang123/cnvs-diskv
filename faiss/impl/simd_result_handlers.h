...
struct SIMDResultHandlerToFloat : SIMDResultHandler {
    ...
    virtual void set_total(size_t total, idx_t* local_ids, float* local_dis){
...
template <class C, bool with_id_map = false>
struct SingleQueryResultHandler : ResultHandlerCompare<C, with_id_map> {
    using T = typename C::T;
    using TI = typename C::TI;
    using RHC = ResultHandlerCompare<C, with_id_map>;
    using RHC::normalizers;
    ...
    SingleQueryResultHandler(size_t ntotal, int64_t k, float* dis, int64_t* ids)
            : RHC(1, ntotal, nullptr), idis(k), iids(k), dis(dis), ids(ids), k(k) {
        // heap_heapify<C>(k, idis.data(), iids.data());   // move this initial
        // operation to func begin();
    }

    void set_total(size_t total, idx_t* local_ids, float* local_dis) override{
        this->k = total;
        pos = 0;
        // TODO resize idis and idis   position 1
        dis = local_dis;
        ids = local_ids;
        idis.resize(k);
        iids.resize(k);
        heap_heapify<C>(k, idis.data(), iids.data());
    }

    void begin(const float* norms) final {
        normalizers = norms;
        // idis and iids are used to store the temperary result of fast scan
        // TODO resize idis and idis    position 2
        // idis.resize(k);
        // iids.resize(k);

        //heap_heapify<C>(k, idis.data(), iids.data());
        // dis and ids are used to store the final result, and they will be used
        // for many loops, so it's necessary to reset it every time.
        //heap_heapify<C>(k, dis, ids);
    }

    // Though there is parameter q, it should always be 0 when this function is
    // called.
    void handle(size_t q, size_t b, simd16uint16 d0, simd16uint16 d1) final {
        if (this->disable) {
            return;
        ...
        this->adjust_with_origin(
                q, d0, d1); // we need offset here instead of real ID, so
                            // with_id_map must be false

        // T* heap_dis = idis.data() + q * k;
        // TI* heap_ids = iids.data() + q * k;
        T* heap_dis = idis.data();
        TI* heap_ids = iids.data();
        ...
        while (lt_mask) {
            ...
            lt_mask -= 1 << j;
            T dis = d32tab[j];
            // TODO it's unnecessary to use heap here. We just need to keep data without order?
            int64_t idx = this->adjust_id(b, j);
            heap_dis[pos] = dis;
            heap_ids[pos] = idx;
            pos++;
    ...
    void end() override {
        T* heap_dis_in = idis.data();
        TI* heap_ids_in = iids.data();
        // TODO unnecessary??
        //heap_reorder<C>(k, heap_dis_in, heap_ids_in);
        float* heap_dis = dis;
        int64_t* heap_ids = ids;
