...
static std::unique_ptr<float[]> compute_residuals(
    ...
    std::unique_ptr<float[]> residuals(new float[n * replicas * d]);
    // TODO: parallelize?
    for (size_t i = 0; i < n; i++) {
        // in order to compute the residual of replicas
        for (int k = 0; k < replicas; k++) {
            if (list_nos[i * replicas + k] < 0)
                memset(residuals.get() + (i * replicas + k) * d,
                       0,
                       sizeof(float) * d);
            else
                quantizer->compute_residual(
                        x + i * d,
                        residuals.get() + (i * replicas + k) * d,
                        list_nos[i * replicas + k]);
        }
    }
    return residuals;
}
...
void IndexIVFPQ::add_core_o(
    ...
    if (n > bs) {
        for (idx_t i0 = 0; i0 < n; i0 += bs) {
            ...
            add_core_o(
                    ...
                    precomputed_idx ? precomputed_idx + i0 * assign_replicas
                                    : nullptr);
        }
        return;
    }
    ...
    // else branch may not be often used, but it's also be modified.
    if (precomputed_idx) {
        idx = precomputed_idx;
    } else {
        idx_t* idx0 = new idx_t[n * num_centroids];
        del_idx.reset(idx0);
        quantizer->assign(n, x, idx0, num_centroids);
        idx = idx0;
    }

    double t1 = getmillisecs();
    std::unique_ptr<uint8_t[]> xcodes(
            new uint8_t[n * num_centroids * code_size]);

    const float* to_encode = nullptr;
    std::unique_ptr<const float[]> del_to_encode;

    if (by_residual) {
        // quantizer is the index of Level1, which compute the redisual between
        // vectors(include replicas) and centroid
        del_to_encode = compute_residuals(quantizer, n, x, idx, num_centroids);
        to_encode = del_to_encode.get();
    } else {
        if(num_centroids != 1){
        // Create a new array with length n * num_centroids and fill it with repeated vectors from x
        float* new_x = new float[n * num_centroids * d];
        for (idx_t i = 0; i < n; ++i) {
            for (idx_t j = 0; j < num_centroids; ++j) {
                std::copy(x + i * d, x + (i + 1) * d, new_x + (i * num_centroids + j) * d);
            }
        }
        del_to_encode.reset(new_x);
        to_encode = del_to_encode.get();
        }
        else
            to_encode = x;
    ...
    pq.compute_codes(to_encode, xcodes.get(), n * num_centroids);

    double t2 = getmillisecs();
    // TODO: parallelize?
    size_t n_ignore = 0;
    for (size_t i = 0; i < n; i++) {
        // key: corresponding cluster id of each vector
        // id:  vector id of itself
        idx_t id = xids ? xids[i] : ntotal + i;

        for (size_t repl = 0; repl < num_centroids; repl++) {
            idx_t key = idx[i * num_centroids + repl];
            if (key < 0) {
            ...
            uint8_t* code =
                    xcodes.get() + (i * num_centroids + repl) * code_size;
