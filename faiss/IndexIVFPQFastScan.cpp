...
void IndexIVFPQFastScan::encode_vectors(
        ...
        uint8_t* codes,
        bool include_listnos) const {      // Add num_centroids parameter
    idx_t num_centroids = assign_replicas; // Add num_centroids variable
    if (by_residual) {
        AlignedTable<float> residuals(n * d * num_centroids);
        for (size_t i = 0; i < n; i++) {
            for (size_t repl = 0; repl < num_centroids; repl++) {
                idx_t list_no = list_nos[i * num_centroids + repl];
                if (list_no < 0) {
                    memset(residuals.data() + (i * num_centroids + repl) * d,
                           0,
                           sizeof(residuals[0]) * d);
                } else {
                    quantizer->compute_residual(
                            x + i * d,
                            residuals.data() + (i * num_centroids + repl) * d,
                            list_no);
                }
            }
        }
        pq.compute_codes(residuals.data(), codes, n * num_centroids);
    } else {
        if (num_centroids != 1) {
            // Allocate memory for the new array to duplicate x
            AlignedTable<float> x_extended(n * d * num_centroids);
            for (size_t i = 0; i < n; i++) {
                for (size_t repl = 0; repl < num_centroids; repl++) {
                    std::copy(
                            x + i * d,
                            x + (i + 1) * d,
                            x_extended.data() + (i * num_centroids + repl) * d);
                }
            }
            pq.compute_codes(x_extended.data(), codes, n * num_centroids);
        } else {
            pq.compute_codes(x, codes, n * num_centroids);
        }
    }
    ...
    if (include_listnos) {
        ...
        for (idx_t i = n - 1; i >= 0; i--) {
            for (size_t repl = 0; repl < num_centroids; repl++) {
                uint8_t* code = codes +
                        (i * num_centroids + repl) * (coarse_size + code_size);
                memmove(code + coarse_size,
                        codes + (i * num_centroids + repl) * code_size,
                        code_size);
                encode_listno(list_nos[i * num_centroids + repl], code);
            }
        }
    }
}
