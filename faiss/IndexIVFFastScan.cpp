...
void IndexIVFFastScan::add_with_ids(
    ...
    std::unique_ptr<idx_t[]> idx(new idx_t[n * num_centroids]);
    quantizer->assign(n, x, idx.get(), num_centroids);
    AlignedTable<uint8_t> flat_codes(n * code_size * num_centroids);
    encode_vectors(n, x, idx.get(), flat_codes.get());
    DirectMapAdd dm_adder(direct_map, n * num_centroids, xids);
    ...
    //    std::cout << "testout4 " << n << std::endl;
    std::vector<idx_t> order(n * num_centroids);
    for (idx_t i = 0; i < n * num_centroids; i++) {
        order[i] = i;
    }
    ...
    idx_t i0 = 0;
    while (i0 < n * num_centroids) {
        idx_t list_no = idx[order[i0]];
        idx_t i1 = i0 + 1;
        while (i1 < n * num_centroids && idx[order[i1]] == list_no) {
            i1++;
        }
        ...
        for (idx_t i = i0; i < i1; i++) {
            // std::cout << "test2 " << i0 << std::endl;
            size_t ofs = list_size + i - i0;
            idx_t id = xids ? xids[order[i] / num_centroids]
                            : ntotal + order[i] / num_centroids;
            dm_adder.add(order[i] / num_centroids, list_no, ofs);
